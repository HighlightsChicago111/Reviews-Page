...
export function AnimatedReviewScoreCard({aggregateRating, reviews, reviewCount, ratingCounts}: Props) {
  ...
  const libraryTotal = useMemo(() => Object.values(ratingCounts).reduce((total, count) => total + count, 0), [ratingCounts])

  useEffect(() => {
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    if (reducedMotion) {
      const immediate = window.setTimeout(() => setIsActive(true), 0)
      return () => window.clearTimeout(immediate)
    }

    const start = window.setTimeout(() => setIsActive(true), 180)
  ...
  return (
    ...
    >
      <div className={`review-summary-panel${isActive ? ' is-active' : ''}`} aria-hidden="true">
