      ...
      <div className="collection-wrap review-directory-grid">
        <aside className="review-filter" aria-label="Review collection filters">
          ...
          <nav className="review-axis-nav" aria-label="Choose how to browse reviews">
            <Link href={{pathname: '/', query: {view: 'rating'}}} aria-current={activeAxis === 'rating' ? 'page' : undefined}>
            ...
            {activeAxis === 'rating' && <div className="review-filter-values">
              <Link href={{pathname: '/', query: {view: 'rating', rating: 'all'}}} aria-current={!activeRating || activeRating === 'all' ? 'page' : undefined}><span>All Google reviews</span><b>{totalExcerpts}</b></Link>
              {starRatings.length > 0 ? starRatings.map((rating) => {
                ...
                return <Link href={{pathname: '/', query: {view: 'rating', rating: rating.toString()}}} aria-current={activeRating === rating.toString() ? 'page' : undefined} key={rating}><span>{rating}-star reviews</span><b>{count}</b></Link>
              }) : <Link href={{pathname: '/', query: {view: 'rating', rating: 'google'}}} aria-current={activeRating === 'google' ? 'page' : undefined}><span>{aggregateRating.toFixed(1)}/5 Google rating</span><b>{totalExcerpts}</b></Link>}
            </div>}
            <Link href={{pathname: '/', query: {view: 'years'}}} aria-current={activeAxis === 'years' ? 'page' : undefined}>
            ...
            {activeAxis === 'years' && <div className="review-filter-values">
              {years.map((year) => {
                ...
                return <Link href={{pathname: '/', query: {view: 'years', year}}} aria-current={activeYear === year ? 'page' : undefined} key={year}><span>{year} reviews</span><b>{count}</b></Link>
              })}
            </div>}
            <Link href={{pathname: '/', query: {view: 'equipment'}}} aria-current={activeAxis === 'equipment' ? 'page' : undefined}>
            ...
            {activeAxis === 'equipment' && <div className="review-filter-values review-filter-values-scroll">
              {stablePages.map((page) => <Link href={`/${page.serviceSlug}`} key={page.serviceSlug}><span>{page.serviceName}</span><b>{page.reviews.length}</b></Link>)}
            </div>}
          </nav>
